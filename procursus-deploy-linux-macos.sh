...
echo "chimera1nv2 deployment script"
echo "(C) 2020, CoolStar. All Rights Reserved"

echo "This script is not ready for public use yet."
exit 1
...
echo '/usr/libexec/firmware' >> chimera-device-deploy.sh
...
echo 'PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/bin/X11:/usr/games dpkg -i org.coolstar.sileo_1.7.6_iphoneos-arm.deb' >> chimera-device-deploy.sh
echo 'echo -n "" > /var/lib/dpkg/available' >> chimera-device-deploy.sh
echo '/Library/dpkg/info/openssh.postinst || true' >> chimera-device-deploy.sh
echo '/Library/dpkg/info/profile.d.postinst' >> chimera-device-deploy.sh
echo 'launchctl load -w /Library/LaunchDaemons/com.openssh.sshd.plist || true' >> chimera-device-deploy.sh
...
curl -L -O https://github.com/coolstar/Chimera-bootstrap/raw/procursus/bootstrap.tar.gz -O https://github.com/coolstar/Chimera-bootstrap/raw/master/org.coolstar.sileo_1.7.6_iphoneos-arm.deb
clear
echo "Copying Files to your device"
echo "Default password is: alpine"
scp -P4444 -o "StrictHostKeyChecking no" -o "UserKnownHostsFile=/dev/null" bootstrap.tar.gz org.coolstar.sileo_1.7.6_iphoneos-arm.deb chimera-device-deploy.sh root@127.0.0.1:/var/root/
clear
